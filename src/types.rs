use crate::{
    albastream::{Error, ErrorKind},
    dynamic_int::{DynamicInteger, vec_from_two_vec},
};
...
impl AlbaTypes {
    ...
    pub fn id(&self) -> u8 {
        match self {
            ...
            AlbaTypes::I128(_) => 12u8,
            AlbaTypes::Geo(_) => 13u8,
        }
    }
    pub fn as_bytes(&self) -> Vec<u8> {
        match self {
            AlbaTypes::String(s) => {
                ...
                b.extend_from_slice(&vec_from_two_vec(
                    &DynamicInteger::from_usize(s.len()).compile(),
                    s.as_bytes(),
                ));
                b
            }
            ...
            AlbaTypes::I128(v) => {
                let mut b = 12u8.to_le_bytes().to_vec();
                b.extend_from_slice(&v.to_le_bytes());
                b
            }
            AlbaTypes::F32(v) => {
            ...
            AlbaTypes::Geo(v) => {
                let mut b = 13u8.to_le_bytes().to_vec();
    ...
    pub fn from_bytes(input: &[u8]) -> Result<(Self, usize), Error> {
        ...
        match id {
            ...
            11 => {
                ...
                Ok((
                    AlbaTypes::Bytes(input[header_size..total_size].to_vec()),
                    total_size,
                ))
            }
            ...
            13 => {
                if input.len() < 17 {
                    return Err(Error::new(ErrorKind::InvalidInput, "Trucated geo"));
                }
                let mut f0 = [0u8; 8];
                let mut f1 = [0u8; 8];

                f0.copy_from_slice(&input[1..9]);
                f1.copy_from_slice(&input[9..17]);

                let f0 = f64::from_le_bytes(f0);
                let f1 = f64::from_le_bytes(f1);

                Ok((AlbaTypes::Geo((f0, f1)), 17))
            }

            _ => Err(Error::new(
                ErrorKind::InvalidInput,
                &format!("Invalid AlbaType id: {}", id),
            )),
        }
    }
}
    ...
    fn digest(&self) -> Vec<u8> {
        ...
        for item in self {
            match item {
                ...
                AlbaTypes::I128(n) => bytes.extend_from_slice(&n.to_le_bytes()),
...
impl ToAlbaAlbaTypes for u128 {
    fn to_alba_alba_types(&self) -> AlbaTypes {
        AlbaTypes::U128(self.clone())
    }
}
impl ToAlbaAlbaTypes for i128 {
    fn to_alba_alba_types(&self) -> AlbaTypes {
...
macro_rules! alba {
    (str: $val:expr) => {
        AlbaTypes::String($val.to_string())
    };
    (string: $val:expr) => {
        AlbaTypes::String($val.to_string())
    };
    (bytes: $val:expr) => {
        AlbaTypes::Bytes($val.to_vec())
    };
    (u8: $val:expr) => {
        AlbaTypes::U8($val)
    };
...
/// # 🚫 **DEPRECATED** 🚫
/// Remain on the database, but it isn't reliable, do not use it in any schema.
...
/// This is the ID of the type **"LIGHT_PASSWORD"** for TytoDB. Use it when creating a new container.
///
/// This belongs to the "Password" group. Types in this group are all intended for the same purpose: storing password hash outputs (e.g., bcrypt or argon2)—not plain passwords.
/// Although designed for storing password hash outputs, it is internally just a slice of bytes, which means you can store anything you want in it.
///
/// **Usage in the database** (both disk and memory):
///   - LIGHT_PASSWORD: 32 bytes
///   - MEDIUM_PASSWORD: 64 bytes
///   - HEAVY_PASSWORD: 128 bytes
pub const LIGHT_PASSWORD: u8 = 17;

/// This is the ID of the type **"MEDIUM_PASSWORD"** for TytoDB. Use it when creating a new container.
///
/// This belongs to the "Password" group. Types in this group are all intended for the same purpose: storing password hash outputs (e.g., bcrypt or argon2)—not plain passwords.
/// Although designed for storing password hash outputs, it is internally just a slice of bytes, which means you can store anything you want in it.
///
...
///   - MEDIUM_PASSWORD: 64 bytes
///   - HEAVY_PASSWORD: 128 bytes
pub const MEDIUM_PASSWORD: u8 = 18;

/// This is the ID of the type **"HEAVY_PASSWORD"** for TytoDB. Use it when creating a new container.
///
/// This belongs to the "Password" group. Types in this group are all intended for the same purpose: storing password hash outputs (e.g., bcrypt or argon2)—not plain passwords.
/// Although designed for storing password hash outputs, it is internally just a slice of bytes, which means you can store anything you want in it.
///
...
///   - MEDIUM_PASSWORD: 64 bytes
///   - HEAVY_PASSWORD: 128 bytes
pub const HEAVY_PASSWORD: u8 = 19;

/// This is the ID of the type **"EMAIL"** for TytoDB. Use it when creating a new container.
///
/// The type is a regular string with a capacity of 320 characters (a generous size for emails) and restricted to ASCII only.  
/// If a non-ASCII input is provided, the library `unidecode` is used to normalize it.
///
/// **Usage in the database** (both disk and memory): 320 bytes
pub const EMAIL: u8 = 20;

/// This is the ID of the type **"Slice0"** for TytoDB. Use it when creating a new container.
///
/// This type is a regular slice of bytes, with a lean capacity suited for small values such as IPv4, IPv6, MAC addresses, or MD5/SHA algorithm outputs.
///
/// **Usage in the database** (both disk and memory):
///    - Slice0: 4 bytes
///    - Slice1: 6 bytes
...
///    - Slice3: 20 bytes
///    - Slice4: 32 bytes
pub const SLICE0: u8 = 26;

/// This is the ID of the type **"Slice1"** for TytoDB. Use it when creating a new container.
///
/// This type is a regular slice of bytes, with a lean capacity suited for small values such as IPv4, IPv6, MAC addresses, or MD5/SHA algorithm outputs.
///
/// **Usage in the database** (both disk and memory):
...
/// This is the ID of the type **"Slice2"** for TytoDB. Use it when creating a new container.
///
/// This type is a regular slice of bytes, with a lean capacity suited for small values such as IPv4, IPv6, MAC addresses, or MD5/SHA algorithm outputs.
///
/// **Usage in the database** (both disk and memory):
...
///    - Slice3: 20 bytes
///    - Slice4: 32 bytes
pub const SLICE2: u8 = 24;

/// This is the ID of the type **"Slice3"** for TytoDB. Use it when creating a new container.
///
/// This type is a regular slice of bytes, with a lean capacity suited for small values such as IPv4, IPv6, MAC addresses, or MD5/SHA algorithm outputs.
///
/// **Usage in the database** (both disk and memory):
...
///    - Slice3: 20 bytes
///    - Slice4: 32 bytes
pub const SLICE3: u8 = 23;

/// This is the ID of the type **"Slice4"** for TytoDB. Use it when creating a new container.
///
/// This type is a regular slice of bytes, with a lean capacity suited for small values such as IPv4, IPv6, MAC addresses, or MD5/SHA algorithm outputs.
///
/// **Usage in the database** (both disk and memory):
...
///    - Slice3: 20 bytes
///    - Slice4: 32 bytes
pub const SLICE4: u8 = 22;

/// This is the ID of the type **"Geo"** for TytoDB. Use it when creating a new container.
///
/// Designed for storing geographic or geometric data.  
/// It is essentially a tuple of two `Float` values (in Rust: `(f64, f64)`).
///
/// **Usage in the database** (both disk and memory): 16 bytes
pub const GEO: u8 = 21;

/// This is the ID of the type **"UInt"** for TytoDB. Use it when creating a new container.
///
/// Essentially a 32-bit unsigned integer.
///
/// **Usage in the database** (both disk and memory): 4 bytes
pub const UINT: u8 = 27;

/// This is the ID of the type **"UBigint"** for TytoDB. Use it when creating a new container.
///
/// Essentially a 64-bit unsigned integer.
///
/// **Usage in the database** (both disk and memory): 8 bytes
pub const UBIGINT: u8 = 28;

/// This is the ID of the type **"NanoInt"** for TytoDB. Use it when creating a new container.
///
/// Essentially an 8-bit signed integer.
///
/// **Usage in the database** (both disk and memory): 1 byte
pub const NANO_INT: u8 = 29;

/// This is the ID of the type **"UNanoInt"** for TytoDB. Use it when creating a new container.
///
/// Essentially an 8-bit unsigned integer.
///
/// **Usage in the database** (both disk and memory): 1 byte
pub const U_NANO_INT: u8 = 30;

/// This is the ID of the type **"Short"** for TytoDB. Use it when creating a new container.
///
/// Essentially a 16-bit signed integer.
///
/// **Usage in the database** (both disk and memory): 2 bytes
pub const SHORT: u8 = 31;

/// This is the ID of the type **"UShort"** for TytoDB. Use it when creating a new container.
///
/// Essentially a 16-bit unsigned integer.
///
/// **Usage in the database** (both disk and memory): 2 bytes
pub const U_SHORT: u8 = 32;

/// This is the ID of the type **"HugeInt"** for TytoDB. Use it when creating a new container.
///
/// Essentially a 128-bit signed integer.
///
/// **Usage in the database** (both disk and memory): 16 bytes
pub const HUGE_INT: u8 = 33;

/// This is the ID of the type **"UHugeInt"** for TytoDB. Use it when creating a new container.
///
/// Essentially a 128-bit unsigned integer.
///
/// **Usage in the database** (both disk and memory): 16 bytes
pub const U_HUGE_INT: u8 = 34;
